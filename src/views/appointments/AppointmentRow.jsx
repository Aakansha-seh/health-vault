...
export function AppointmentRow({ appt, patient, doctor, clinic, onUpdate }) {
  ...
  return (
    ...
    >
      {/* Patient name + reason */}
      <div>
        <p style={{ fontSize: 14, fontWeight: 500, color: C.primary }}>
      ...
      <div style={{ fontSize: 13, color: C.text }}>
        <RelativeDate date={appt.date} />
      </div>

      {/* Time */}
      <span style={{ fontSize: 13, color: C.text }}>{formatTime(appt.time)}</span>
      ...
      {appt.status === 'scheduled' ? (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <Button
            ...
            onClick={handleGCal}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill={C.white}>
